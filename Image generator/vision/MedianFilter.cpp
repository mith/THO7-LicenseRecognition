...
namespace vis {
	...
	MedianFilter::MedianFilter(unsigned int ratio) : pRatio(ratio) {}

	void MedianFilter::ExecuteAlgorithm(Image *image_src, Image *image_dst) {
		...
		for (unsigned int y = pRatio; y < image_src->fields.height - pRatio; ++y) {
			for (unsigned int x = pRatio; x < image_src->fields.width - pRatio; ++x) {
				...
				image_dst->SetPixel_R(x, y, r_hist.GetMedian(9));
				image_dst->SetPixel_G(x, y, g_hist.GetMedian(9));
				image_dst->SetPixel_B(x, y, b_hist.GetMedian(9));
